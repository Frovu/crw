...
from events.misc.plots import epoch_collision, custom_plot
...
def _custom_plot():
	...
	if interval[1] - interval[0] <= 0 or int(interval[1]) - int(interval[0]) > 3600 * 24 * 369:
		raise ValueError('bad interval')

	data = custom_plot(interval, definitions, feid_id)
	return { 'data': data }
...
def _epoch_collision():
	...
	if not times or not interval or not series:
		raise ValueError('malformed request')
	if interval[1] - interval[0] <= 0 or int(interval[1]) - int(interval[0]) > 1200:
...
def _coverage():
	...
	for c in ['donki_flares', 'donki_cmes', 'solarsoft_flares', 
		'solardemon_flares', 'solardemon_dimmings', 'lasco_cmes', 'cactus_cmes', 'r_c_icmes', 'solen_holes']:
...
def _fetch_source(progr, source, tstamp):
	...
	elif source == 'cactus_cmes':
		cactus_cme.fetch(progr)
