...
def select(interval: [int, int], query=None):
	res = requests.get(CRDT_HOST + f'/api/omni?from={interval[0]}&to={interval[1]}&query={",".join(query)}', verify=False)
...
def ensure_prepared(interval: [int, int]):
	res = requests.get(CRDT_HOST + f'/api/omni/ensure?from={interval[0]}&to={interval[1]}', verify=False)
	if res.status_code != 200:
