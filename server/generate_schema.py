...
with open(TARGET, 'w') as f:
	...
	def export_const(lbl, val):
		text = json.dumps(val, indent=4)
		text = re.sub(r'"([a-z_]+)":', r'\1:', text) 
		f.write(f'export const {lbl} = {text.replace('"', "'")} as const;\n\n')

	export_const('sourceLinks', SOURCE_LINKS)
	export_const('sourceLabels', SOURCE_LABELS)
	export_const('sourceColumnOrderingOptions', SRC_COL_ORDERING_OPTIONS)
	export_const('tablesColumns', { tbl: [col.sql_name for col in cols] for tbl, cols in ALL_TABLES.items() })
	...
	export_const('omniSourceOptions', { g.value.upper(): [s.value.upper() for s in sc] for g, sc in omni.GROUP_SOURCES.items()}) # type: ignore
