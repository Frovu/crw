	...
	return <div style={{ height: '100%', width: '100%', display: 'flex' }}>
		<div style={{ width: 'calc(100% - 224px)', display: 'flex', gap: 4, flexDirection: 'column', padding: '4px 8px' }}>
			<div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
				...
				<button style={{ padding: '0 8px' }} onClick={() => resetColors()}>Reset colors</button>
			</div>
			<div style={{ flexGrow: 1, overflowY: 'scroll',
				...
				onClick={() => setPicking(col)}>
					<div style={{ display: 'inline-block', border: '1px solid var(--color-grid)', height: '1em', width: '2em',
						backgroundColor: color(col) }}></div>
					{color(col) !== rgbaToHexa(getDefaultColor(col)) ? '*' : ''}{col}
				</div>)}
			</div>

		</div>
		{picking && old && <div style={{ overflow: 'auto', marginRight: 6 }}>
