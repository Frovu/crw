...
function parseW0(text: string) {
	...
	for (const line of allLines.slice(numbersIdx)) {
		...
		values.push(...split.slice(0, 12).map((v) => (/9999/.test(v) ? null : parseFloat(v))));
		if (split.length > 12) {
...
export default function LoadFile({ path }: { path: string }) {
	...
	return (
		<>
			<button style={{ padding: '2px 16px' }} onClick={() => setOpen(true)}>
				Load a file
			</button>
			{open && (
				<>
					...
					<div className="Popup" style={{ transform: 'unset', left: 8, top: 8, padding: '0 2em' }}>
						...
						{data && data[0].length > 0 && (
							<button style={{ marginTop: 16, padding: '2px 16px' }} onClick={() => mutation.mutate()}>
								Upload {targetVar} to server (overwrite)
							</button>
						)}
