...
export default function Neutron() {
	...
	const query = useQuery({
		queryKey: ['manyStations', queryStations, interval],
		queryFn: async () => {
	...
	const revertMutation = useMutation({
		mutationFn: (revId: number) => apiPost('neutron/revert', { id: revId }),
	...
	const partialDataState = useMemo(() => {
		...
		const averages = data.map((sd) => {
			const s = sd
					...
					.slice()
					.sort((a, b) => a - b),
				mid = Math.floor(sd.length / 2);
			return s.length % 2 === 0 ? s[mid] : (s[mid] + s[mid + 1]) / 2;
		});
		const sortedIdx = Array.from(stations.keys())
	...
	const dataState = useMemo(() => {
		...
		if (hRev) {
			...
			for (const time of hRev.rev_time) hIindicators[data[0].indexOf(time)] = level;
		}
		...
		return {
			...partialDataState,
			plotData: [...partialDataState.plotData.slice(0, -2), hIindicators, aIindicators],
		};
	...
	const [efficiency, efficiencyInput] = useEfficiencyInput(!primeStation || !selection, () => {
		if (!dataState || !primeStation || !selection) return 1;
		...
		const lEff = data[min] / left,
			rEff = data[max] / right;
		return (lEff + rEff) / 2;
	});

	const addCorrection = useCallback(
		(station: string, fromIndex: number, values: number[]) => {
			setCorrections((corr) => {
				if (!dataState) return {};
	...
	useEventListener('keydown', (e: KeyboardEvent) => {
		if (e.code === 'Escape') return openPopup(null);
		if (activePopup) return e.stopImmediatePropagation();
		if (e.code === 'KeyF') {
			openPopup('refetch');
					...
					<div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
						...
						{dataState && (
							<div style={{ margin: '0 0 4px 16px' }}>
								Primary station:{' '}
								<select
									style={{ color: primeStation ? 'var(--color-green)' : 'var(--color-text)' }}
									value={primeStation ?? 'none'}
									onChange={(e) =>
										navigation.setState((st) => ({
											...st,
											chosen:
												e.target.value === 'none'
													? null
								...
								{showRevisions.map((rev) => (
									<div
										...
										style={{
											...
											backgroundColor: hoveredRev === rev.id ? 'var(--color-area)' : 'var(--color-bg)',
										}}
									...
									>
										<p style={{ margin: 0 }}>
											...
											<button
												...
												onClick={() => revertMutation.mutate(rev.id)}
											>
												Revert{rev.reverted_at != null ? 'ed' : ''}
											</button>
										</p>
...
function useEfficiencyInput(autoDisabled: boolean, auto: () => number) {
	type S = { text: string; value: number; div: number | null };
	type A = [{ action: 'value' | 'div' | 'checkbox' | 'auto'; value?: any }];
	const [{ text, value, div }, dispatch] = useReducer<S, A>(
		(st, { action, value: aValue }) => {
			if (action === 'value') {
	...
	useEventListener('keydown', (e: KeyboardEvent) => {
		if (e.code === 'KeyA') dispatch({ action: 'auto' });
	});
	...
	return [
		...
		<div style={{ display: 'inline-block' }}>
			<label>
				{' '}
				Div
				<input type="checkbox" onChange={(e) => dispatch({ action: 'checkbox', value: e.target.checked })} />{' '}
			</label>
			Eff=
			<input
				style={{ width: '6ch', borderColor: 'var(--color-border)', textAlign: 'center' }}
				type="text"
				value={text}
				onChange={(e) => dispatch({ action: 'value', value: e.target.value })}
			/>
			{div != null && (
				<span>
					&nbsp;/{' '}
					<input
						...
						step={1}
						value={div.toFixed(0)}
						onChange={(e) => dispatch({ action: 'div', value: e.target.valueAsNumber })}
					/>
				</span>
			)}
