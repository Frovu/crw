...
function Menu({ params, setParams }: ContextMenuProps<CollisionOptions>) {
	...
	const set = <T extends keyof CollisionOptions>(k: T, val: CollisionOptions[T]) => setParams({ [k]: val });
		...
		<div className="Group">
			...
			<div>
				...
				<select
					...
					onChange={(e) => set('timeColumn', e.target.value as any)}
				>
...
function Panel() {
	...
	const samples = useMemo(
		() =>
			[sample0, sample1, sample2].map((name) => {
				if (!name) return null;
