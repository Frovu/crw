...
function Panel() {
	...
	const plot = useMemo(() => {
		...
		const dirColor = (mpa: number) => {
			if (mpa >= 45 && mpa < 135) return colors.east;
			if (mpa >= 135 && mpa < 225) return colors.south;
			if (mpa >= 225 && mpa < 315) return colors.west;
			return colors.north;
		};
		const data = [[], ...(query.data?.map((c) => [0, 1].map((i) => c.ht.map((r) => r[i]))) ?? [])] as any;
		const options = () => {
