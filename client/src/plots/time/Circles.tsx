...
import { usePlot, type Onset } from '../../events/core/util';
...
function PlotCircles({ params: initParams, settingsOpen }: { params: CirclesPlotParams; settingsOpen?: boolean }) {
	...
	const plotComponent = useMemo(() => {
		...
		const legendValue = (seriesIdx: number) => (u: uPlot) => {
			if (u.data == null || seriesIdx !== hoveredRect?.sidx) return '';
			const d = u.data[hoveredRect.sidx] as any;
			if (hoveredRect.didx >= d[0].length) return '';
			const stIdx = d[3][hoveredRect.didx],
				lon = d[1][hoveredRect.didx].toFixed(2);
		...
		const setSelect = (u: uPlot, val: number) =>
			u.setSelect({
		...
		const options: () => uPlot.Options = () => ({
			...
			hooks: {
				...
				ready: [
					...
					(u) => {
						if (interactive) u.over.style.setProperty('cursor', 'pointer');
						...
						u.over.addEventListener('mousemove', (e) => {
							if (isDragged) {
								...
								if (currentBase < u.scales.x.min!) currentBase = u.scales.x.min!;
								if (currentBase > u.scales.x.max! - 86400) currentBase = u.scales.x.max! - 86400;
							}
							setSelect(u, currentBase);
						});
						...
						{...{
							options: {
								...
								scales: {
									...
									idx: {
										range: (u, min, max) => [-0.1, Math.max(6, max * 1.2)],
									},
								},
								axes: [
									{
...
function CirclesParamsInput({ params, setParams }: { params: CirclesPlotParams; setParams: (p: CirclesPlotParams) => void }) {
	const callback = (what: string) => (value: any) => {
		if (what === 'days') {
			...
			if (!value) return setParams({ ...params, onsets: undefined });
	...
	return (
		<div className="Settings">
			<div style={{ textAlign: 'left', paddingLeft: '4em' }}>
				<b>Settings</b>
			</div>
			Ending date:
			<ValidatedInput
			...
			<div style={{ lineHeight: '2em' }}>
				...
				<br />
				<Checkbox text="Fix amplitude scale" k="fixAmplitudeScale" />
				<br />
				<Checkbox text="Linear size scaling" k="linearSize" />
				<br />
				<Checkbox text="Show second plot" k="rsmExtended" />
			</div>
		</div>
	);
}
...
export function PlotCirclesStandalone() {
	...
	if (params.theme) document.documentElement.setAttribute('main-theme', params.theme);
	...
	return (
		<div style={{ position: 'relative', height: '98vh', width: '100vw' }}>
			...
			<button
				className="Button"
				style={{ bottom: 0, lineHeight: 1, left: 10, ...(settingsOpen && { color: 'var(--color-active)' }) }}
				onClick={() => setOpen((o) => !o)}
			>
				S
			</button>
			<input
				style={{ position: 'absolute', fontSize: 15, bottom: 0, left: 46, width: '5em', borderRadius: 6 }}
				type="number"
				min="-9"
				max="9"
				step=".05"
			...
			></input>
			<input
				style={{ position: 'absolute', fontSize: 15, bottom: 0, left: 46 + 80, width: '5em', borderRadius: 6 }}
				type="number"
				min="-99"
				max="99"
				step="1"
...
function Panel() {
	...
	return (
		<>
			...
			<a
				style={{ backgroundColor: 'var(--color-bg)', position: 'absolute', top: 0, right: 4 }}
				href="./ros"
				target="_blank"
				onClick={() => window.localStorage.setItem('plotRefParams', JSON.stringify(params))}
			>
				link
			</a>
		</>
