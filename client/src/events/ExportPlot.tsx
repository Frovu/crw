...
type ActualOverrides = Omit<PlotsOverrides, 'textTransform'> & {
  textTransform?: TranformEntry[];
};
...
export const usePlotExportSate = create<PlotExportState>()(
  persist(
    immer((set) => ({
      ...
      setScale: (id, k, scl) =>
        set(({ overrides, perPlotScales, savedScales }) => {
          if (overrides.scalesParams?.[k])
            Object.assign(overrides.scalesParams?.[k], scl);
          if (perPlotScales) savedScales[id] = overrides.scalesParams ?? {};
        }),
      setPerPlotMode: (id, val) =>
        set((state) => {
          state.perPlotScales = val;
          if (val && state.savedScales[id])
...
export function renderOne(nodeId: string) {
  ...
  const controlsPresent = !!Object.values(list[active]?.items).find(
    (i) => i?.type === 'ExportControls'
  );
  const opts = {
...
async function doRenderPlots() {
  ...
  for (const [nodeId, node] of Object.entries(layout)) {
    const [x, y, w, h] = (['x', 'y', 'w', 'h'] as const).map(
      (d) => scale * node[d]
    );
...
function PreviewPanel() {
  ...
  return (
    <div
      style={{ padding: 2, height: '100%', overflow: 'auto' }}
      onClick={() => setShow(!show)}
    >
      <span style={{ padding: 2 }}>
        preview plots (may be slow){' '}
        <input type="checkbox" checked={show} readOnly />
      </span>
      {show && renderTime && (
        <div
          style={{
...
export function ExportableUplot({
  ...
  const { scalesParams, textTransform } = usePlotExportSate(
    (st) => st.overrides
  );
  const { items } = useLayout();
  const controlsPresent = !!Object.values(items).find(
    (i) => i?.type === 'ExportControls'
  );
              ...
              queueMicrotask(() =>
                usePlotExportSate.setState((state) => {
                  ...
                  for (const scl in u.scales) {
                    const { positionValue, scaleValue }: CustomScale =
                      u.scales[scl];
                    if (positionValue && scaleValue)
                      state.plots[layout.id].scales[scl] = {
                        ...positionValue,
                        ...scaleValue,
                      };
                  }
...
export function TextTransformContextMenu({
  detail: { action },
}: {
  detail: TextTransformMenuDetail;
}) {
  const {
  ...
  const query = useQuery(
    ['textTransforms'],
    () => apiGet<{ list: TransformSet[] }>('events/text_transforms'),
    {
      onError: logError,
    }
  );
  ...
  const presets = useMemo(() => {
    ...
    return query.data.list.sort(
      (a, b) => (a.author === login ? -1 : 1) - (b.author === login ? -1 : 1)
    );
  }, [query.data, login]);
  ...
  const removeMut = useMutation(
    (name: string) => apiPost('events/text_transforms/remove', { name }),
    {
      onSuccess: (msg, name) => {
            ...
            {presets.map(
              ({
                ...
                >
                  ...
                  {isPub && (
                    <div style={{ color: color('text-dark'), fontSize: 12 }}>
                      (public)
                    </div>
                  )}
                  {author === login ? (
  ...
  const nameInvalid =
    selected == null &&
    (nameInput === '' ||
      presets.find((p) => p.author === login && p.name === nameInput));
    ...
    <div className="Group">
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            setSelected(
              e.target.value === '__new' ? null : parseInt(e.target.value)
            )
          }
        >
        ...
        <div>
          ...
          <input
            ...
            style={{
              width: 222,
              marginLeft: 4,
              borderColor: color(nameInvalid ? 'active' : 'bg'),
            }}
            value={nameInput}
...
function ControlsPanel() {
  ...
  async function doExportPlots(download: boolean = false) {
    ...
    a.download = `feid_figure_${w.toString().replace('.', 'p')}_${
      useCm ? 'cm' : 'in'
    }.png`;
  ...
  const scales: { [k: string]: ScaleParams } = Object.assign(
    {},
    ...plotsScales
  );
  const effectiveScales = Object.entries(scales).map(([scl, params]) => ({
    scl,
    ...(scalesParams?.[scl] ?? params),
  }));

  const fontPx = Math.round((width / inches / 72) * fontSize * scale);
  const setOverride =
    (scl: string, param: 'min' | 'max' | 'bottom' | 'top') =>
      ...
      <div>
        <div style={{ display: 'flex', gap: 4, color: color('white') }}>
          <button
            ...
            onClick={() => doExportPlots()}
          >
            Open png
          </button>
          <button
            style={{ flex: 1, minWidth: 'max-content' }}
            onClick={() => doExportPlots(true)}
          >
            Download
          </button>
        </div>
        {devicePixelRatio !== 1 && (
          <div style={{ fontSize: 12, color: color('red') }}>
            pixelRatio ({devicePixelRatio.toFixed(2)}) != 1, export won't work
            as expected, press Ctrl+0 if it helps
          </div>
        )}
        <div
          style={{ display: 'flex', flexWrap: 'wrap', gap: 4, paddingTop: 4 }}
        >
          <span>
            <label
              title={`Actual font size = ${fontPx}px = ${(
                (fontPx / scale / width) *
                inches *
                72
              ).toFixed(2)}pt`}
          ...
          <span>
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setInches(e.target.valueAsNumber / (useCm ? 2.54 : 1))
                }
              />
            </label>
            <label style={{ padding: '0 4px' }}>
              ...
              <input
                ...
                type="checkbox"
                checked={useCm}
                onChange={(e) => setUseCm(e.target.checked)}
              />
            </label>
            ,
            <label
              style={{ paddingLeft: 4 }}
              title="Approximate resolution when shrinked to specified size"
            >
              <select
        ...
        <div style={{ color: color('text-dark'), paddingTop: 2 }}>
          ...
          {((width * height * 0.74 * (scale - 1.2)) / 1024 / 1024).toFixed(2)}{' '}
          MB
        </div>
            ...
            .map(({ scl, min, max, bottom, top }) => {
              ...
              return (
                <div
                  ...
                  style={{
                    cursor: 'pointer',
                    color: !active ? color('text-dark') : 'unset',
                  }}
                  onClick={(e) =>
                    !(e.target instanceof HTMLInputElement) &&
                    (active
                      ? removeScale(plotId, scl)
                      : addScale(plotId, scl, scales[scl]))
                  }
                >
                  <span
                    style={{ textDecoration: !active ? 'unset' : 'underline' }}
                  >
                    {scl}
                  </span>
                  <input
        ...
        >
          ...
          >
            ...
            <button
              ...
              style={{
                padding: '0 6px',
                color: color(!textTransform?.length ? 'text-dark' : 'skyblue'),
              }}
          ...
          {textTransform?.map(({ search, replace, id, enabled }) => (
            <div
              ...
              onMouseDown={(e) =>
                !(e instanceof HTMLInputElement) && setDragging(id)
              }
            >
              <label style={{ minWidth: 'max-content' }}>
                <input
                  ...
                  checked={!!enabled}
                  onChange={(e) =>
                    setTransform(id, { enabled: e.target.checked })
                  }
                />
                RegEx
              </label>
              ...
              >
                ...
                <input
                  ...
                  value={replace}
                  onChange={(e) =>
                    setTransform(id, { replace: e.target.value })
                  }
                />
                <span
...
export function PlotIntervalInput({
  step: alterStep,
  solar,
}: {
  step?: number;
    ...
    >
      ...
      <input
        ...
        onChange={(e) =>
          ...
          set(target, [e.target.valueAsNumber, right])
        }
      />
      /{' '}
      <input
        ...
        onChange={(e) =>
          !isNaN(e.target.valueAsNumber) &&
          set(target, [left, e.target.valueAsNumber])
        }
      />{' '}
      h
    </div>
...
function ControlsMenu() {
  return (
    <>
      <button
        onClick={openContextMenu('textTransform', { action: 'save' }, true)}
      >
        Save replaces
      </button>
      <button
        onClick={openContextMenu('textTransform', { action: 'load' }, true)}
      >
        Load replaces
      </button>
    </>
