...
export const useGenericState = create<GenericState>()(
	immer((set) => ({
		...defaultState,
		setGeneric: (g) =>
			set((state) => {
		...
		setPoint: (k, val) =>
			set(({ params }) => {
				const type = EXTREMUM_OP.includes(val as any) ? 'extremum' : val.endsWith('sws') ? 'sw_structure' : 'event';
				const inp = params[k];
		...
		setPointHours: (k, val) =>
			set(({ params: { [k]: point } }) => {
				if (point) point.hours_offset = val;
			}),
		setPointSeries: (k, val) =>
			set(({ params: { [k]: point } }) => {
				if (point?.type === 'extremum') point.series = val;
			}),
		setPointStruct: (k, val) =>
			set(({ params: { [k]: point } }) => {
...
export function fromDesc(desc: ColumnDef) {
	const width = (() => {
		switch (desc.type) {
			case 'enum':
				return Math.max(5, ...desc.enum!.map((el) => el.length));
			case 'time[]':
				return 17;
			case 'time':
				return 17;
...
export const fetchTable = async (entity: string) => {
	...
	const data = res.data.map((row) => [
		...columns.map((c, i) => {
			if (row[i] == null || (c.type.endsWith('[]') && (row[i] as any).length < 1)) return null;
			if (c.type === 'time') {
