...
function Menu({ params, setParams }: ContextMenuProps<Params>) {
	...
	const Select = ({ k, opts }: { k: 'mode' | 'prefer' | 'src' | 'enlilVar'; opts: readonly string[] }) => (
		<select className="Borderless" value={para[k]} onChange={(e) => setParams({ [k]: e.target.value as any })}>
			{opts.map((m) => (
	...
	return (
		<div className="Group">
			<div>
				Mode: <Select k={'mode'} opts={MODES} />
			</div>
			{mode !== 'WSA-ENLIL' && (
				<div>
					Prefer flare: <Select k={'prefer'} opts={PREFER_FLR} />
				</div>
			)}
			{mode === 'WSA-ENLIL' && (
				<div>
					Param: <Select k={'enlilVar'} opts={ENLIL_OPTS} />
				</div>
			)}
			{mode === 'SDO' && (
				<div>
					Src: <Select k={'src'} opts={SDO_SRC} />
				</div>
			)}
			{mode === 'SDO' && (
				<div>
...
function EnlilView({ id }: { id: number | null }) {
	...
	const query = useQuery({
		queryKey: ['enlil', id],
		queryFn: () => (id == null ? id : apiGet<{ filename: string }>('events/enlil', { id })),
	});
		...
		<>
			...
			<div style={{ transform: `scale(${size / 400})`, transformOrigin: 'top left', height: 400, width: 400, overflow: 'hidden' }}>
				...
				<iframe
					...
					src={dMN_FLR + `&flare_id=${id}`}
					scrolling="no"
					width={Math.max(size, 408)}
					height={Math.max(size, 408)}
				/>
			</div>
		</>
	);
...
function Panel() {
	...
	const cme = (() => {
		if (cursor?.entity === 'CMEs') return rowAsDict(cmes.data[cursor.row], cmes.columns);
	...
	const flare = (() => {
		if (cursor?.entity === 'flares') return rowAsDict(flares.data[cursor.row], flares.columns);
