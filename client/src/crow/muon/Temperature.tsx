...
export default function TemperatureApp() {
	...
	const query = useQuery({
		...
		staleTime: 36e5,
		refetchInterval: ({ state: { data } }) => (data?.status === 'busy' ? 500 : false),
	});
	...
	const plotComponent = useMemo(() => {
		...
		return (
			<UplotReact
				{...{
					options: { ...size, ...plotOptions(), legend: { mount: (upl, el) => legendContainer?.append(el) } },
					data: data as any,
					onCreate: setUplot,
				}}
			/>
					...
					JSON.stringify(
						{
							...
							info:
								'Atmospheric temperature data of NCEP/NCAR Reanalysis project (https://psl.noaa.gov/data/gridded/data.ncep.reanalysis.html) interpolated for scpecific location and for 1 hour time resolution. Obtained at: ' +
			...
			<div style={{ height: '100%', display: 'grid', gridTemplateColumns: '360px 1fr', gap: 4, userSelect: 'none' }}>
				<div>
					...
					<div style={{ padding: '8px 0 4px 8px' }}>
						...
						{Object.entries(query.data?.downloading ?? {}).map(([year, progr]) => (
							<div key={year}>
				...
				<div ref={(el) => setContainer(el)} style={{ position: 'relative' }}>
					...
					<div style={{ position: 'absolute' }}>{plotComponent}</div>
				</div>
			</div>
		</>
	);
